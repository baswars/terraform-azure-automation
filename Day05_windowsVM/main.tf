...
resource "azurerm_network_security_group" "nsg" { # This block defines an Azure Network Security Group resource, 
  ...
  security_rule {
    name                       = "RDP" # This rule allows inbound SSH traffic on port 22 from any source to any destination. 
    priority                   = 100
    direction                  = "Inbound"
    access                     = "Allow"
    protocol                   = "Tcp"
    source_port_range          = "*"
    destination_port_range     = "3389" # This rule allows inbound RDP traffic on port 3389 from any source to any destination.
...
resource "azurerm_windows_virtual_machine" "windowsvm" { # This block defines an Azure Windows Virtual Machine resource.
  name                = "windowsvm"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
